...
[ ! -e /var/flash/panic2 ] && mknod /var/flash/panic2 c "$tffs_major" 96
# (log)files
for l in $(find /etc/*.pkg /var/log/* /etc/.config /etc/options.cfg /tmp/flash/mod/rc.* /var/flash/debug.cfg /var/flash/crash.log /var/flash/panic /var/flash/panic2 /var/env.* /proc/partitions /proc/bus/usb/devices /mod/etc/conf/cs_cams.cfg /mod/etc/conf/tbflex.cfg -type f -o -type c); do
	cat $l > $SUPDIR/${l##*/} 2>&1
done
